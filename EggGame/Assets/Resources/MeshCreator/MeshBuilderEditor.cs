...
public class MeshBuilderEditor : Editor
{
    ...
    public void OnSceneGUI()
    {
        ...
        if (Event.current.control && Event.current.shift)
        {
            ...
            Handles.CircleHandleCap(120, p, Quaternion.identity, 0.1f, EventType.Repaint);
        ...
        if (t.Corners.Count > 0)
        {
            ...
            if (minDist <= minPointerDist && minDist < minVertDist * 0.25f && minVertDist > 0.1f && t.showOutline && !Event.current.shift)
            {
                ...
                Handles.DotHandleCap(113, GetClosestPointOnLineSegment(lineVerts[minDistIndex].position, lineVerts[nextIndex].position, mousePos), Quaternion.identity, 0.03f * t.handleScale * handleScale, EventType.Repaint);
            ...
            else if (minVertDist < 0.1f * t.handleScale * handleScale)
            {
                ...
                else if ((t.showHandles && !Event.current.shift) && !(Event.current.control && Event.current.shift))
                {
                    ...
                    Handles.DotHandleCap(117, t.transform.position + t.transform.rotation * v, Quaternion.identity, 0.08f * t.handleScale * handleScale, EventType.Repaint);
            ...
            if ((t.showHandles && !Event.current.shift) && !(Event.current.control && Event.current.shift))
            {
                for (int i = 0; i < t.Corners.Count; i++)
                {
                    ...
                    if (t.Corners[i].isBezier)
                    {
                        ...
                        if (t.Corners[lastIndex].isBezier)
                        {
                            cp0 = Handles.FreeMoveHandle(t.transform.position + t.transform.rotation * v0, Quaternion.identity, .03f * t.handleScale * handleScale, new Vector3(.5f, .5f, .5f), Handles.DotHandleCap);
                        ...
                        if (t.Corners[nextIndex].isBezier)
                        {
                            cp1 = Handles.FreeMoveHandle(t.transform.position + t.transform.rotation * v1, Quaternion.identity, .03f * t.handleScale * handleScale, new Vector3(.5f, .5f, .5f), Handles.DotHandleCap);
                    ...
                    Vector3 pos = Handles.FreeMoveHandle(t.transform.position + t.transform.rotation * v, Quaternion.identity, .06f * t.handleScale * handleScale, new Vector3(.5f, .5f, .5f), Handles.DotHandleCap);
                ...
                if (GUIUtility.hotControl == lastHotControl)
                {
                    if (hotID != -1)
                    {
                        if (t.verticalSymmetry != Event.current.alt && Vector3.Distance(vSymPos, t.Corners[hotID].position) <= t.symmertryDistance * 10)
                        {
                            ...
                            Handles.CircleHandleCap(222, t.transform.rotation * v + t.transform.position, Quaternion.identity, 0.047f * t.handleScale * handleScale, EventType.Repaint);
                        }
